...
-- the project). This file adds EVERYTHING else the app expects: every
-- migration under supabase/migrations/ plus organizations, reels, scout clips,
-- and tournament rooms. There is no other file to run.
--
-- Afterwards run supabase/SUPABASE_AUDIT.sql (or GET /api/health with the
-- x-seed-secret header) — every row must read OK.
-- =============================================================================
...
-- ── Profile role must be nullable (20260608000001) ───────────────────────────
-- schema.sql ships role NOT NULL DEFAULT 'player'. With that default, new
-- accounts skip onboarding and protect_profile_role() then refuses to ever let
-- them pick organizer. Drop the default, allow NULL, and gate the trigger on a
-- NULL role instead of an empty full_name.
...
-- ── Game cover images (20260611000000) ─────────────────────────────────────
...
-- ── Tournament platform: numbers, rosters, goals, honors (20260712000000) ───
...
drop policy if exists "Rosters are publicly viewable" on public.tournament_team_members;
create policy "Rosters are publicly viewable"
  on public.tournament_team_members for select using (true);

drop policy if exists "Players join rosters as themselves" on public.tournament_team_members;
...
drop policy if exists "Players leave or captains remove" on public.tournament_team_members;
...
drop policy if exists "Goals are publicly viewable" on public.tournament_goals;
create policy "Goals are publicly viewable"
  on public.tournament_goals for select using (true);

drop policy if exists "Organizers record goals" on public.tournament_goals;
...
drop policy if exists "Awards are publicly viewable" on public.tournament_awards;
...
-- ── Tournament room chat (20260708000000) ───────────────────────────────────
-- The table must exist before the roster-access policies below; earlier
-- versions of this file only shipped the policies, which fails on a project
-- that never ran the tournament_rooms migration.
...
-- =============================================================================
-- ORGANIZATIONS + PHONE + REELS + SCOUT SHORTLISTS
-- (20260613000000_organizations, 20260613000000_player_reels)
-- Formerly the separate RUN_REELS_AND_ORGS.sql; folded in so one file is enough.
-- =============================================================================
...
-- =============================================================================
-- SCOUT CLIPS (20260614000000_scout_clips)
-- =============================================================================
...
drop policy if exists "Scout clip videos are publicly readable" on storage.objects;
create policy "Scout clip videos are publicly readable"
  on storage.objects for select using (bucket_id = 'scout-clips');

drop policy if exists "Users upload their own scout clip videos" on storage.objects;
...
drop policy if exists "Users update their own scout clip videos" on storage.objects;
...
drop policy if exists "Users delete their own scout clip videos" on storage.objects;
...
-- =============================================================================
-- Profile usernames (login can resolve @username → email)
-- =============================================================================
alter table public.profiles
